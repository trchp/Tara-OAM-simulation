...
phi12 = 0;
phi13 = pi/12;
phi2space=linspace(-pi/6, pi/6, 9); % +
puritys1 = getpuritys(apw, aph, txwidth, phi11, phi2space, z, l1, l2, wavelength);
puritys2 = getpuritys(apw, aph, txwidth, phi12, phi2space, z, l1, l2, wavelength);
puritys3 = getpuritys(apw, aph, txwidth, phi13, phi2space, z, l1, l2, wavelength);

plot(phi2space, puritys1, '-x', 'DisplayName', "phi1 = " + phi11);
hold on;
plot(phi2space, puritys2, '-x','DisplayName', "phi1 = " + phi12);
plot(phi2space, puritys3, '-x', 'DisplayName', "phi1 = " + phi13);
legend;

xlabel('Phi2 (angle of incidence from tx2)'); % Label for x-axis
ylabel('average purity for l1 at rx1 with steering for phi1'); % Label for y-axis
title('Average purity vs angular misalignment'); % Title for the plot



%% helper function get puritys

function puritys = getpuritys(apw, aph, txwidth, phi1, phi2space, z, l1, l2, wavelength)
puritys = zeros(9,1);
for i=1:9
...
DemodSCat1 = SteerCat1.*exp(-1*j*l1*atan2(Ye1, Xe1-range1Center));
DemodSCat2 = SteerCat2.*exp(-1*j*l2*atan2(Ye2, Xe2 - range2Center));

SteerField = steer(phi2, RxEfield, Xe2, wavelength);
...
%half1CatPurity = findPurity(SteerCat1, l1, Xe1, Ye1, range1Center);
%half2CatPurity = findPurity(SteerCat2, l2, Xe2, Ye2, range2Center);
half1CatPurity = findPurity(DemodSCat1, 0, Xe1, Ye1, range1Center);
half2CatPurity = findPurity(DemodSCat2, 0, Xe2, Ye2, range2Center);
puritys(i,1) = (half1CatPurity+half2CatPurity)/2;
